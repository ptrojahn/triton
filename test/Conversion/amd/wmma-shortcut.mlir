...
#wmma = #ttg.amd_wmma<{version = 1, warpsPerCTA = [2, 1], isTranspose = false}>
#wmmaT = #ttg.amd_wmma<{version = 1, warpsPerCTA = [2, 1], isTranspose = true}>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 2 : i32, "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK-LABEL: wmma_dot_cvt_f16_wmma
  tt.func public @wmma_dot_cvt_f16_wmma(%arg0: tensor<32x32xf16, #wmmaT>) {
    // CHECK-NOT: store
    // CHECK-NOT: load
    ...
    // CHECK: llvm.return
    %0 = ttg.convert_layout %arg0 : tensor<32x32xf16, #wmmaT> -> tensor<32x32xf16, #dotop0>
    tt.return
  }
}